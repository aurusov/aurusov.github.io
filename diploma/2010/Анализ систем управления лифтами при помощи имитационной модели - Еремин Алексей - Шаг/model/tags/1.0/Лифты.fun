...
$Function СостПустЛифт: such_as Лифты.Состояние
$Type = algorithmic
$Parameters
	Требуемый_этаж: such_as Этаж.Номер
	Текущий_этаж  : such_as Этаж.Номер
$Body
	if Требуемый_этаж ==  Текущий_этаж result = ПосадкаВозможна
	if Требуемый_этаж <> Текущий_этаж result = Едет
$End

$Function НаправПустЛифт: such_as Лифты.НаправлениеЛ
$Type = algorithmic
$Parameters
	Состояние     : such_as Лифты.Состояние
	Требуемый_этаж: such_as Лифты.Требуемый_этаж
	Текущий_этаж  : such_as Лифты.Текущий_этаж
	кнопка        : such_as Этаж.кнопка_вниз //сумма ( - кнопка_вниз + кнопка_вверх)
$Body
	if Состояние == Едет result = (Требуемый_этаж - Текущий_этаж)/IAbs(Требуемый_этаж - Текущий_этаж)
	if Состояние == ПосадкаВозможна and кнопка == -1 result = -1
	if Состояние == ПосадкаВозможна and кнопка ==  0 result = -1
	if Состояние == ПосадкаВозможна and кнопка == +1 result = +1
/*
Здесь с помощью порядка строк задается, что приоритет направления вниз выше, чем вверх.
Но, похоже, что вероятность конфликтов в реальных случаях будет близка нулю.
!кнопка используется как сумма из-за ограничение РДО на количество параметров!
*/
$End
